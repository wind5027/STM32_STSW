...
#ifndef __FS_INT_H__               // Avoid recursive and multiple inclusion
#define __FS_INT_H__
...
typedef int         FS_CB_CHECKDISK           (FS_VOLUME  * pVolume,   FS_DISK_INFO * pDiskInfo, void * pBuffer, U32 BufferSize, int MaxRecursionLevel, FS_ON_CHECK_DISK_ERROR_CALLBACK * pfOnError);
...
struct FS_FILE {                  // Describes the file handle structure
  ...
  U8            InUse;
  U8            AccessFlags;
  U8            DataModified;     // Set to 1 if a write operation changed the file data.
  FS_FILE     * pNext;
};
...
FS_WRITEMODE FS__GetFileWriteModeEx(FS_VOLUME * pVolume);
...
int         FS__STORAGE_RefreshSectors(FS_VOLUME * pVolume, U32 SectorIndex, U32 NumSectors, void * pBuffer, U32 NumBytes);
...
#endif
#if FS_FAT_USE_FSINFO_SECTOR
  U8              UseFSInfoSector;  // Allows the user to enable/disable at runtime the use of FSInfo sector to get the number of free clusters.
#endif
#if FS_MAINTAIN_FAT_COPY
  U8              MaintainFATCopy;  // Allows the user to enable/disable at rumtime the update of the second allocation table.
#endif
} FS_GLOBAL;
...
int         FS_MAP_CheckDisk           (FS_VOLUME  * pVolume,   FS_DISK_INFO * pDiskInfo, void * pBuffer, U32 BufferSize, int MaxRecursionLevel, FS_ON_CHECK_DISK_ERROR_CALLBACK * pfOnError);
void        FS_MAP_UpdateFile          (FS_FILE    * pFile);
int         FS__GetFSType              (FS_VOLUME  * pVolume);
...
int    FS_LB_FreeSectorsPart     (FS_PARTITION    * pPart,   U32 SectorNo, U32 NumSectors);
...
int    FS_LB_ReadPart            (FS_PARTITION    * pPart,   U32 SectorNo,                       void * pBuffer, U8 Type);
...
int    FS_LB_WritePart           (FS_PARTITION    * pPart,   U32 SectorNo,                 const void * pBuffer, U8 Type, U8 WriteToJournal);
