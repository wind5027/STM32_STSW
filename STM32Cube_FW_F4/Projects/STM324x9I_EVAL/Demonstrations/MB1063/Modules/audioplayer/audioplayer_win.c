...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { WINDOW_CreateIndirect,    "Audio Player", ID_FRAMEWIN_INFO,     0,   0,   640, 454, 0, 0x64, 0 },
  { LISTVIEW_CreateIndirect,  "Listview",     ID_WAVFILE_LIST,      280, 06,  350, 440, 0, 0x0,  0 },  
  ...
  { TEXT_CreateIndirect,      "Duration",     ID_ELAPSED_TIME,      35,  225, 250, 250,  0, 0x0,  0 },
...
static const char            acMask_wav[] = ".wav";
  ...
  if (res == FR_OK)
  {
    ...
    while (1)
    {
      ...
      if (pWavList->ptr < FILEMGR_LIST_DEPDTH)
      {
        ...
        {
          if((strstr(fn, ".wav")) && (AUDIOPLAYER_hWin != 0))
          {
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    hItem = BUTTON_CreateEx(22,  352, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_STOP_BUTTON);
    WM_SetCallback(hItem, _cbButton_stop);    
       
    hItem = BUTTON_CreateEx(89, 352, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_PREVIOUS_BUTTON);
    WM_SetCallback(hItem, _cbButton_previous);   
    
    hItem = BUTTON_CreateEx(148, 345, 50,  50, pMsg->hWin, WM_CF_SHOW, 0, ID_PLAY_BUTTON);
    WM_SetCallback(hItem, _cbButton_play);
    
    hItem = BUTTON_CreateEx(222, 352, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_NEXT_BUTTON);
    WM_SetCallback(hItem, _cbButton_next);
    
    hItem = WM_GetClientWindow(pMsg->hWin);
    hItem = BUTTON_CreateEx(20, 410, 50,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_WAV_BUTTON);
    WM_SetCallback(hItem, _cbButton_open); 
    
    hItem = BUTTON_CreateEx(80, 410, 50,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_ADD_BUTTON);
    WM_SetCallback(hItem, _cbButton_add);      
       
    hItem = BUTTON_CreateEx(235,  410, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_CLOSE_BUTTON);
  ...
  case WM_PAINT:
    ...
    GUI_DrawGradientRoundedV(15, 190, 270, 325, 30, 0x00A77009, 0x00FFFF00);
    DrawRect3D(15, 340, 255, 60);
    DrawRect3D(15, 405, 255, 40);
