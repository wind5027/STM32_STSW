...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { WINDOW_CreateIndirect,   "Video Player", ID_FRAMEWIN_INFO,    0,   0,   640, 454, 0, 0x64, 0 },
  { LISTVIEW_CreateIndirect, "Listview",     ID_VIDEO_LIST,       470, 05,  165, 390, 0, 0x0,  0 },   
  { SLIDER_CreateIndirect,   "Slider",      ID_PROGRESS_SLIDER,   05,  373, 435, 20,  0, 0x0,  0 },
}; 
...
static const char                acMask_emf[] = ".emf";
  ...
  if (res == FR_OK)
  {
    ...
    while (1)
    {
      ...
      if (pVideoList->ptr < FILEMGR_LIST_DEPDTH)
      {
        ...
        {
          if((strstr(fn, ".emf")) && (VIDEOPLAYER_hWin != 0))
          {
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    hItem = BUTTON_CreateEx(360,  410, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_STOP_BUTTON);
    WM_SetCallback(hItem, _cbButton_stop);    
    
    hItem = BUTTON_CreateEx(152, 410, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_PREVIOUS_BUTTON);
    WM_SetCallback(hItem, _cbButton_previous);     
    
    hItem = BUTTON_CreateEx(217, 403, 50,  45, pMsg->hWin, WM_CF_SHOW, 0, ID_PLAY_BUTTON);
    WM_SetCallback(hItem, _cbButton_play);
    
    hItem = BUTTON_CreateEx(295, 410, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_NEXT_BUTTON);
    WM_SetCallback(hItem, _cbButton_next);
    
    hItem = BUTTON_CreateEx(87,  413, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_REPEAT_BUTTON);
    WM_SetCallback(hItem, _cbButton_repeat);     
      
    hItem = BUTTON_CreateEx(475, 413, 70,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_OPEN_BUTTON);
    WM_SetCallback(hItem, _cbButton_open); 

    hItem = BUTTON_CreateEx(535, 413, 70,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_ADD_BUTTON);
    WM_SetCallback(hItem, _cbButton_add);     
    
    hItem = BUTTON_CreateEx(600,  410, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_CLOSE_BUTTON);
    WM_SetCallback(hItem, _cbButton_close);    
    
    hItem = BUTTON_CreateEx(441,  370, 25,  25, pMsg->hWin, WM_CF_SHOW, 0, ID_FULL_SCREEN_BUTTON);
  ...
  case WM_PAINT:
    ...
    DrawRect3D(05, 401, 460, 50);
    DrawRect3D(470, 401, 165, 50); 
...
case WM_NOTIFY_PARENT:
    ...
    case ID_ADD_BUTTON: 
      switch(NCode) {
      case WM_NOTIFICATION_RELEASED:
        if((chooser_openfile == 0) && (chooser_addfolder == 0) && (chooser_addfolder == 0))
        {
          pFileInfo->pfGetData = k_GetData;
          pFileInfo->pMask = acMask_emf;     
