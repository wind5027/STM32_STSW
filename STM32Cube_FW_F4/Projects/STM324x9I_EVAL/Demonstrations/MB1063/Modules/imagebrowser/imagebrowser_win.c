...
{
  { WINDOW_CreateIndirect,  "Window",           ID_WINDOW_0,       0,   0,  640, 454, 0, 0x0, 0 },
  { LISTBOX_CreateIndirect, "List",             ID_IMAGE_LIST,     445, 15, 180, 380, 0, 0x0, 0 }
};
  ...
  if (res == FR_OK)
  {
    ...
    while (1)
    {
      ...
      if (pImageList->ptr < FILEMGR_LIST_DEPDTH)
      {
        ...
        {
          if((strstr(fn, ".jpg")) || (strstr(fn, ".bmp")))
          {
  ...
  case WM_TIMER:
   ...
   if(Id == ID_SLIDER_TIMER)
   {
     if(pImageList->ptr > 1)
     {
       ...
       if(strstr((char *)pImageList->file[file_pos].name, ".bmp"))
       {
         IMAGE_Type = IMAGE_TYPE_BMP;
       }
       else if(strstr((char *)pImageList->file[file_pos].name, ".jpg"))
       {
...
static void _cbDialog(WM_MESSAGE * pMsg)
{
  ...
  case WM_INIT_DIALOG:
    ...
    hItem = BUTTON_CreateEx(138, 405, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_PREVIOUS_BUTTON);
    WM_SetCallback(hItem, _cbButton_previous);
    
    hItem = BUTTON_CreateEx(185, 398,  50,  50, pMsg->hWin, WM_CF_SHOW, 0, ID_SLIDE_BUTTON);
    WM_SetCallback(hItem, _cbButton_play);
    slideshow_state = OFF;
    
    hItem = BUTTON_CreateEx(247, 405, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_NEXT_BUTTON);
    WM_SetCallback(hItem, _cbButton_next);
    
    
    hItem = BUTTON_CreateEx(546, 408, 50,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_ADD_BUTTON);
    WM_SetCallback(hItem, _cbButton_add);
    
    hItem = BUTTON_CreateEx(474, 408, 50,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_OPEN_BUTTON);
    WM_SetCallback(hItem, _cbButton_open);
    
    hItem = BUTTON_CreateEx(400, 407, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_SETTINGS_BUTTON);
    WM_SetCallback(hItem, _cbButton_settings);
    
    hItem = BUTTON_CreateEx(20, 407, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_CLOSE_BUTTON);
 ...
 case WM_PAINT:
    DrawRect3D(15, 396, 420, 53);
    DrawRect3D(445, 396, 170, 53);
  ...
  case WM_NOTIFY_PARENT:
    ...
    if (NCode == WM_NOTIFICATION_CHILD_DELETED)
    {
      ...
      {
        if((strstr(pFileInfo->pRoot, ".jpg")) || (strstr(pFileInfo->pRoot, ".bmp")))
        {
    ...
    case ID_OPEN_BUTTON: 
      ...
      case WM_NOTIFICATION_CLICKED:
        break;
      case WM_NOTIFICATION_RELEASED:
        if((chooser_addfolder == 0) && (chooser_addfolder == 0))
        {
