...
extern uint16_t ADCConvertedValueX;
extern uint16_t ADCConvertedValueX1;
...
void ADCx_DMA_IRQHandler(void)
{
  ...
  if(abs((ADCConvertedValueX >>4) - (ADCConvertedValueX1 >>4)) > 4)
  {
    if((PrevXferDone)  && (hUSBDDevice.dev_state == USBD_STATE_CONFIGURED))
    {
      SendBuffer[1] = (uint8_t)(ADCConvertedValueX >>4);
      
      USBD_CUSTOM_HID_SendReport (&hUSBDDevice, SendBuffer, 2);
      
      ADCConvertedValueX1 = ADCConvertedValueX;
      PrevXferDone = 0;
    }
  }
}
