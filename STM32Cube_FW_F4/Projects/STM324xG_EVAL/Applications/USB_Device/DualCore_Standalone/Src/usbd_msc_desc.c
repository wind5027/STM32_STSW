...
#define USBD_MSC_SERIALNUMBER_HS_STRING   "00000000001A"
#define USBD_MSC_PRODUCT_FS_STRING        "Mass Storage in FS Mode"
#define USBD_MSC_SERIALNUMBER_FS_STRING   "00000000001B"
...
uint8_t *USBD_MSC_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  {    
    USBD_GetString((uint8_t *)USBD_MSC_SERIALNUMBER_HS_STRING, USBD_MSC_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_MSC_SERIALNUMBER_FS_STRING, USBD_MSC_StrDesc, length);    
  }
  return USBD_MSC_StrDesc;
}
